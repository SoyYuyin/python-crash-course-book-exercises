# Using your latest Restaurant class, store it in a module.
# Make a separate file that imports Restaurant. Make a Restaurant
#  instance, and call one of Restaurant’s methods to show that
# the import statement is working properly.

from restaurant import Restaurant

el_jefe = Restaurant('el jefe', 'mexican desayunos')

el_jefe.describe_restaurant()
el_jefe.open_restaurant()
